...
interface EdicionPropiedadDialogProps {
...
export function EdicionPropiedadDialog({
  residencia,
  open,
  onOpenChange,
  onSuccess,
}: EdicionPropiedadDialogProps) {
  const [loading, setLoading] = useState(false);
  const [ciudades, setCiudades] = useState<Ciudad[]>([]);
  const [imagenes, setImagenes] = useState<File[]>([]);
  const [previewUrls, setPreviewUrls] = useState<string[]>([]);
  const [imagenActual, setImagenActual] = useState<string | undefined>(
  ...
  const [formData, setFormData] = useState<PropiedadFormData>({
    ...
    precioPorNoche: residencia.precioPorNoche.toString(),
    ciudadSedeId: residencia.ciudadSedeId?.toString() || "",
  });
  ...
  useEffect(() => {
    const fetchCiudades = async () => {
      try {
        ...
        // Si no hay ciudadSedeId, buscar por nombre
        if (!residencia.ciudadSedeId && residencia.ciudadNombre) {
          const ciudad = mockCiudades.find(
            (c) => c.nombre === residencia.ciudadNombre
          );
          if (ciudad) {
            setFormData((prev) => ({
              ...prev,
              ciudadSedeId: ciudad.id.toString(),
            }));
          }
        }
      } catch (error) {
  ...
  // Resetear el formulario cuando cambia la residencia
  useEffect(() => {
    setFormData({
      titulo: residencia.titulo,
      descripcion: residencia.descripcion,
    ...
    setImagenActual(residencia.imagenUrl);
    setImagenes([]);
    setPreviewUrls([]);
    setErrors({});
  }, [residencia]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(`Actualizando propiedad ${residencia.id}:`, payload);

      await Api.put(`/residencias/${residencia.id}`, payload);
      ...
      if (imagenes.length > 0) {
        ...
        console.log(`Subiendo nuevas imágenes para propiedad ${residencia.id}`);
        await Api.post(
          `/residencias/${residencia.id}/imagenes`,
          formDataImages,
          {
      ...
      if (response?.status === 400 && data && typeof data === "object") {
        ...
        toast.error("Debes iniciar sesión para editar propiedades");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl">Editar propiedad</DialogTitle>
          <DialogDescription>
            Actualiza la información de tu propiedad en MtyHostal
          </DialogDescription>
        </DialogHeader>
...
export default EdicionPropiedadDialog;
